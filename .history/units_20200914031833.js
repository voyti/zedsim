...
export class Units {
  ...
  getUnitTypeBehaviors(type) {
    ...
    const civilianBehaviors = [
      ...
      },
      {
        ...
        action: (unit) => this.moveUnitInDirection(unit, this.getDirectionTowardsClosest(), CIVILIAN_ROAM_DISTANCE),
      }, {
        type: 'escape',
        priority: 2,
  ...
  getUnitsInRange(unitTypes, thisUnit, range) {
    ...
    const unitsWithDistances = _.map(relevantUnits, (unit) => ({
      unit,
      distanceTo: this.positioningService.getDistanceBetweenPoints(thisUnit, unit),
    }));
  ...
  getDirectionTowardsClosest(thisUnit, unitType) {
    const units = this.getUnitsInRange(unitType, thisUnit);
    const closestUnit = units[0];
    const upwardsRef = {x: thisUnit.sprite.x, y: thisUnit.sprite.y - 10};

    return this.positioningService.calculateAngle(upwardsRef, thisUnit.sprite, closestUnit.sprite);
  }
