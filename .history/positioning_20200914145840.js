...
export class Positioning {
  ...
  getRotatedPointByOuterReference (outerP, centerP, angleDegrees) {
  ...
  correctPointToNearestWalkable(point, unit) {
    ...
    if (!this.checkIfPointInsideObstacle(point)) {
      if (unit.type === 'civilian') debugging.placeDebuggingMark(this.appBoard.stage, point.x, point.y, 'dest', 0x99ff99);
      return point;
    } else {
      ...
      if (xPossibility < yPossibility && xPossibility > 0) {
        if (unit.type === 'civilian') debugging.placeDebuggingMark(this.appBoard.stage, xPossibility, point.y, 'Cdest(x)', 0x99ff99);

        return { x: xPossibility, y: point.y };
      } else if (yPossibility > 0) {
        if (unit.type === 'civilian') debugging.placeDebuggingMark(this.appBoard.stage, point.x, yPossibility, 'Cdest(y)', 0x99ff99);
